# +++++++++++++++++++++++++++++++ WITHOUT LIVE PREDICTION +++++++++++++++++++++++++++++++++++++
# """
# ========================================================================
# HEATWAVE RISK PREDICTION SYSTEM
# Step 6: Interactive Dashboard (Streamlit)
# ========================================================================
# Run with:  streamlit run step6_dashboard.py

# Dashboard Panels:
#   🏠 Home      – Today's risk for all 3 cities at a glance
#   📈 Trends    – Historical risk score + compound events time-series
#   🔍 Predict   – Manual input form for custom predictions
#   📊 Compare   – Cross-city comparison & statistics
#   🗺  Map       – Risk map of the three cities (folium)
# ========================================================================
# """
...
# # ── PAGE CONFIG ───────────────────────────────────────────────────────────
# st.set_page_config(
#     page_title  = "Heatwave Risk Prediction",
#     page_icon   = "🌡",
#     layout      = "wide",
...
# @st.cache_resource
# def load_model(city_tag="all"):
#     path = os.path.join(MODEL_DIR, f"classifier_{city_tag}.pkl")
#     if not os.path.exists(path):
#         path = os.path.join(MODEL_DIR, "classifier_all.pkl")
#     if not os.path.exists(path):
#         return None, None
#     with open(path, "rb") as f:
#         bundle = pickle.load(f)
#     reg_path = os.path.join(MODEL_DIR, f"regressor_{city_tag}.pkl")
#     reg = None
...
# <style>
#   .risk-card {
#     border-radius: 12px; padding: 18px; margin: 6px;
#     text-align: center; font-family: Arial, sans-serif;
#   }
#   .metric-big { font-size: 2.4rem; font-weight: 800; }
...
# # ── SIDEBAR ───────────────────────────────────────────────────────────────
# with st.sidebar:
#     st.image("https://img.icons8.com/color/96/000000/thermometer.png", width=60)
#     st.title("Heatwave Risk System")
#     st.caption("India — 3-City Prediction")
...
#     ])
#     st.markdown("---")
#     selected_city = st.selectbox("City Filter", ["All"] + CITIES)
#     date_range = st.date_input("Date Range",
#         value=(datetime(2020, 1, 1), datetime(2024, 12, 31)))

# df = load_data()
# if df is None:
#     st.error("⚠ Data not found. Run Steps 1–4 first to generate processed data.")
#     st.stop()

# # Apply filters
# if selected_city != "All":
#     df_view = df[df["city"] == selected_city].copy()
# else:
#     df_view = df.copy()
...
# # ════════════════════════════════════════════════════════════════════════
# #  PAGE 1: TODAY'S RISK OVERVIEW
# # ════════════════════════════════════════════════════════════════════════
# if "Overview" in page:
#     st.title("🌡 Today's Heatwave Risk — All Cities")
#     st.caption(f"Based on latest available data")

#     cols = st.columns(3)
#     for col, city in zip(cols, CITIES):
#         latest = df[df["city"] == city].dropna(subset=["risk_label"]).iloc[-1]
#         risk   = latest["risk_label"]
#         score  = latest["composite_score"]
#         temp   = latest["temp_max"]
...
#           <div style="font-size:1.4rem;font-weight:700">{city}</div>
#           <div style="font-size:3rem">{emoji}</div>
#           <div class="metric-big">{risk}</div>
#           <div class="metric-sub">Score: {score:.0f} / 100</div>
#           <hr>
#           <div>🌡 Temp Max: <b>{temp:.1f}°C</b></div>
#           <div>🥵 Heat Index: <b>{hi:.1f}°C</b></div>
#           <div>💨 AQI: <b>{aqi:.0f}</b></div>
#           <div>📅 {latest['date'].strftime('%d %b %Y') if hasattr(latest['date'], 'strftime') else latest['date']}</div>
#         </div>
#         """, unsafe_allow_html=True)
...
#     severe_days = (df_view["risk_level"] == 3).sum()
#     high_days   = (df_view["risk_level"] == 2).sum()
#     compound    = df_view["triple_compound"].sum()
#     avg_score   = df_view["composite_score"].mean()
#     m1.metric("Severe Days", f"{severe_days}", help="risk_level=3")
#     m2.metric("High Risk Days", f"{high_days}")
#     m3.metric("Triple Compound Events", f"{compound}")
#     m4.metric("Avg Composite Score", f"{avg_score:.1f}")

#     # Mini trend chart
#     fig = px.line(df_view, x="date", y="composite_score",
#                   color="city", title="Composite Risk Score — All Cities",
#                   color_discrete_map={"Delhi":"#3498DB","Hyderabad":"#E74C3C","Nagpur":"#2ECC71"})
#     fig.add_hline(y=50, line_dash="dot", line_color="orange", annotation_text="High Risk Threshold")
#     fig.add_hline(y=75, line_dash="dot", line_color="red",    annotation_text="Severe Threshold")
...
#         "Risk Score Timeline", "Temperature Analysis",
#         "Compound Events", "Monthly Heatmaps"
#     ])

#     with tab1:
#         city_sel = st.selectbox("City", CITIES, key="t1city")
#         sub = df[df["city"] == city_sel]

#         fig = make_subplots(rows=2, cols=1, shared_xaxes=True,
#                              subplot_titles=("Composite Risk Score", "Raw Temperature + AQI"))

#         # Colour by risk level
#         for lvl, label in RISK_LABELS.items():
#             mask = sub["risk_level"] == lvl
#             fig.add_trace(go.Scatter(
#                 x=sub[mask]["date"], y=sub[mask]["composite_score"],
#                 mode="markers", marker=dict(color=RISK_COLORS[lvl], size=3),
#                 name=label), row=1, col=1)

#         fig.add_trace(go.Scatter(x=sub["date"], y=sub["temp_max"],
#             name="Temp Max", line=dict(color="#E74C3C", width=1)), row=2, col=1)
#         fig.add_trace(go.Scatter(x=sub["date"], y=sub["aqi"],
#             name="AQI", line=dict(color="#9B59B6", width=1),
#             yaxis="y3"), row=2, col=1)

#         fig.update_layout(height=500, title=f"{city_sel} — Risk Timeline")
#         st.plotly_chart(fig, use_container_width=True)

#     with tab2:
#         fig = px.box(df_view, x="month", y="heat_index",
#                      color="city", title="Monthly Heat Index Distribution")
#         st.plotly_chart(fig, use_container_width=True)

#         fig2 = px.scatter(df_view.sample(min(3000, len(df_view))),
#                           x="temp_max", y="heat_index",
#                           color="risk_label", size="aqi",
#                           color_discrete_map={
#                               "Low":"#2ECC71","Moderate":"#F1C40F",
#                               "High":"#E67E22","Severe":"#E74C3C"},
#                           title="Heat Index vs Temperature (bubble = AQI)")
#         st.plotly_chart(fig2, use_container_width=True)

#     with tab3:
#         compound_cols = ["compound_heat_aqi","compound_heat_drought",
#                           "compound_heat_humidity","triple_compound"]
#         nice = ["Heat+AQI","Heat+Drought","Heat+Humidity","Triple Compound"]
#         grp  = df_view.groupby(["city","month"])[compound_cols].sum().reset_index()
#         grp.columns = ["city","month"] + nice

#         fig = px.bar(grp.melt(id_vars=["city","month"],
#                                value_vars=nice, var_name="Type", value_name="Days"),
#                      x="month", y="Days", color="Type", facet_col="city",
#                      barmode="group", title="Monthly Compound Event Frequency")
#         st.plotly_chart(fig, use_container_width=True)

#     with tab4:
#         city_h = st.selectbox("City", CITIES, key="hmap")
#         sub_h  = df[df["city"] == city_h].copy()
...
#                         title=f"{city_h} — Avg Risk Score Heatmap (Month × Week)",
#                         labels={"x":"Week of Year","y":"Month","color":"Score"})
#         st.plotly_chart(fig, use_container_width=True)
...
#     st.title("🔍 Custom Daily Prediction")
#     st.caption("Enter today's environmental readings to get a risk prediction.")

#     c1, c2 = st.columns([1, 1])
#     with c1:
#         city_p  = st.selectbox("City", CITIES)
...
#         aqi     = st.slider("AQI", 0, 500, 120)

#     if st.button("🔮 Predict Risk", type="primary"):
#         # Quick feature computation (no full pipeline for UI speed)
#         def quick_heat_index(T, RH):
#             TF = T * 9/5 + 32
#             HI = (-42.379 + 2.049*TF + 10.143*RH - 0.225*TF*RH
#                   - 0.00684*TF**2 - 0.0548*RH**2)
#             return (HI - 32) * 5/9

#         hi   = quick_heat_index(t_max, humidity)
#         is_hw= t_max >= 40
#         score_t  = 0 if t_max<35 else 25 if t_max<38 else 50 if t_max<40 else 75 if t_max<45 else 100
#         score_hi = 0 if hi<27 else 25 if hi<32 else 50 if hi<41 else 75 if hi<54 else 100
#         score_aq = 0 if aqi<=50 else 15 if aqi<=100 else 40 if aqi<=200 else 60 if aqi<=300 else 80 if aqi<=400 else 100
#         score_dr = 40 if rainfall < 1 else 0
#         c_heat_aqi= int(t_max>=38 and aqi>=200)
#         c_heat_dry= int(t_max>=38 and rainfall<1)
#         c_heat_hum= int(t_max>=38 and humidity>=60)
#         compound  = min(100, c_heat_aqi*30 + c_heat_dry*25 + c_heat_hum*20)

#         final_score = (0.35*score_t + 0.25*score_hi + 0.20*score_aq +
#                        0.10*score_dr + 0.10*compound)
#         if t_max >= 45: final_score = max(final_score, 75)
#         elif t_max >= 40: final_score = max(final_score, 50)

#         level = 0 if final_score<25 else 1 if final_score<50 else 2 if final_score<75 else 3
#         label = RISK_LABELS[level]
#         color = RISK_COLORS[level]
#         emoji = {0:"🟢",1:"🟡",2:"🟠",3:"🔴"}[level]

#         st.markdown(f"""
#         <div class="risk-card risk-{label.upper()}" style="max-width:500px;margin:auto">
#           <div style="font-size:1.2rem;font-weight:700">{city_p} — Predicted Risk</div>
#           <div style="font-size:4rem">{emoji}</div>
#           <div class="metric-big">{label}</div>
#           <div class="metric-sub">Composite Score: {final_score:.1f} / 100</div>
#           <hr>
#           <div>🥵 Heat Index: <b>{hi:.1f}°C</b></div>
#         </div>
#         """, unsafe_allow_html=True)
...
#             r=[score_t, score_hi, score_aq, score_dr, compound],
#             theta=["Temperature","Heat Index","AQI","Drought","Compound"],
#             fill="toself", fillcolor=f"rgba({','.join(str(int(c,16)) for c in [color[1:3],color[3:5],color[5:]])},0.3)",
#             line_color=color, name="Risk Score",
#         ))
#         fig.update_layout(polar=dict(radialaxis=dict(range=[0,100])),
#                            title="Risk Pillar Breakdown",
...
#     tab1, tab2, tab3 = st.tabs(["City Comparison", "Seasonal Patterns", "Risk Statistics"])
...
#         fig = px.bar(stats, x="city",
#                      y=["Severe_Days","High_Days"],
#                      barmode="group", title="Severe & High Risk Days by City",
#                      color_discrete_sequence=["#E74C3C","#E67E22"])
#         st.plotly_chart(fig, use_container_width=True)

#     with tab2:
#         seasonal = df.groupby(["city","season"])["composite_score"].mean().reset_index()
#         fig = px.bar(seasonal, x="season", y="composite_score",
#                      color="city", barmode="group",
#                      title="Average Risk Score by Season & City",
...
#             default=["temp_max","heat_index","composite_score"])
#         if selected_metrics:
#             fig = px.violin(df, x="city", y=selected_metrics[0],
#                             color="risk_label",
#                             color_discrete_map={
#                                 "Low":"#2ECC71","Moderate":"#F1C40F",
#                                 "High":"#E67E22","Severe":"#E74C3C"},
#                             box=True, title=f"{selected_metrics[0]} distribution by city & risk")
#             st.plotly_chart(fig, use_container_width=True)



# +++++++++++++++++++++++++++++++ WITH LIVE PREDICTION (VERSION 1) (GETTING DIFFERENT VALUES IN TERMINAL & DASHBOARD FOR SAME DATE) +++++++++++++++++++++++++++++++++++++
# """
# ========================================================================
# HEATWAVE RISK PREDICTION SYSTEM
...
#     history  = (df_hist[df_hist["city"]==city][raw_cols]
#                 .dropna().tail(40).copy())
...
#         m3.metric("Triple Compound",   int(df_view["triple_compound"].sum()))
...
# +++++++++++++++++++++++++++++++ WITH LIVER PREDICTION (VERSION 2)  (ONLY TODAY & TOMORROW FORECASTING) +++++++++++++++++++++++++++++++
...
#                     hist14["date"] = hist14["date"].dt.strftime("%d %b")
#                     hist14.columns = ["Date","Tmax°C","AQI","Risk","Score"]
#                     st.dataframe(hist14.set_index("Date"), width="stretch")
...
# ++++++++++++++++++++++++++++++ WITH LIVE PREDICTION (VERSION 3) (MULTIDAY FORECASTING) +++++++++++++++++++++++++++++++++++++++++
